...
3. 任何可继续使用的创作成果都必须通过 submit_candidate 提交为普通项目文件的新建、修改或删除清单。提交后系统会自动写入，因此改动范围和完整内容必须准确。
...
const TASK_PROMPTS = {
  ...
  import_novel:
    "导入作品由后台分析流程整理人物、关系、时间、认知、故事线、伏笔和文风；不要再生成需要作者逐项确认的建档候选。作者发现错误后通过聊天直接修正。",
  generate_characters:
  ...
  refresh_memory:
    "只记录正式章节中新增或改变的事实并附来源。正文冲突时以正文为准。新结构项目应交由增量分析流程更新图谱，不要求作者逐项确认。使用 submit_memory_changes 提交兼容结果。",
  review:
...
function classifyTask(instruction, targetType = "") {
  ...
  if (/评审|审稿|检查|一致性|事实冲突|认知泄漏|吃书/.test(text)) return "review";
  ...
  if (/未来.*章|接下来.*章|近期.*章|近期章节|章节计划|(?:规划|计划).*章|章.*(?:规划|计划)/.test(text)) return "plan_chapters";
  ...
  if (/为什么|怎么|如何|哪些|哪里|是谁|是否|请解释|分析.*原因|关系.*(?:如何|怎样)|[吗呢][？?]?$/.test(text)) return "query";
