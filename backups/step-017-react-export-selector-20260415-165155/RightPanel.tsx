...
import { UsersIcon, GlobeIcon, FeatherIcon, ChevronDownIcon, ChevronRightIcon, FileTextIcon, BookIcon, PlusIcon, DownloadIcon, ExternalLinkIcon, SparklesIcon } from 'lucide-react';
        ...
        {contentExpanded && (
          <div className="px-3 py-2 space-y-1">
            <div className="flex gap-2 px-3 py-2">
              ...
              >
                <PlusIcon size={11} />
                {currentChapterNumber ? `续写第 ${currentChapterNumber} 章` : '续写当前章'}
              </button>
            </div>
              ...
              {outlineExpanded && (
                <div className="ml-7 flex flex-col gap-1">
                  <button
                    ...
                    className="w-[calc(100%-1.75rem)] flex items-center gap-2 px-3 py-2 rounded-lg transition-colors"
                    style={{ background: 'transparent' }}
                    onMouseEnter={(e) => (e.currentTarget as HTMLElement).style.background = '#EEF3FF'}
                    onMouseLeave={(e) => (e.currentTarget as HTMLElement).style.background = 'transparent'}
                  >
                    <SparklesIcon size={12} color="#4A7CF7" />
                    <span style={{ color: '#4A7CF7' }} className="text-xs">{outline ? '重新生成蓝图' : '生成蓝图'}</span>
                  </button>
                  <button
                    onClick={() =>
                      onLoadDoc({
