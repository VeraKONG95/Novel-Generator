import { Chapter, Character, NovalMemory, NovalProject, Project, RecentProjectSummary, WorldSetting } from '../types';
...
export function buildHomeProjects(
  ...
  const cards = recentProjects.map(recentSummaryToCard);
  ...
  return [currentCard, ...cards.filter((item) => item.filePath !== currentPath)];
}
