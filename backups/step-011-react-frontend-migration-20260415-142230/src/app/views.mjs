  ...
  return `
    <div class="shell">
      ...
      <main class="workspace">
        ...
        ${state.recoveryNotice ? renderRecoveryNotice(state.recoveryNotice) : ""}
        ...
        <section class="status-bar">
          ...
          <div class="status-meta">
            <div id="autosave-badge" class="badge ${autosaveBadgeClass(state.autosave)}">${escapeHtml(
              autosaveBadgeText(state.autosave)
            )}</div>
...
function renderRecoveryNotice(notice) {
  return `
    <section class="recovery-banner ${escapeHtml(notice.kind || "info")}">
      <div>
        <h3>${escapeHtml(notice.title || "恢复提示")}</h3>
        <div class="subtle">${escapeHtml(notice.text || "")}</div>
      </div>
      <button class="secondary" data-action="dismiss-recovery">知道了</button>
    </section>
  `;
}

function autosaveBadgeClass(autosave) {
  const phase = autosave?.phase || "idle";
  return `badge-${phase}`;
}

function autosaveBadgeText(autosave) {
  const phase = autosave?.phase || "idle";
  ...
  if (phase === "saved" && lastSavedAt) {
    return `自动保存 ${shortTime(lastSavedAt)}`;
  }
  if (phase === "error") {
    return "自动保存失败";
  }
          ...
          currentChapter
            ? `
              <div class="stack">
                ...
                ${
                  state.lastRewriteReview?.chapterId === currentChapter.id
                    ? renderRewriteReview(state.lastRewriteReview, currentChapter)
                    : ""
                }
                <div class="field">
                  <label>正文</label>
...
function renderSettings(state) {
  ...
  const exportOptions = state.project.exportOptions;
  return `
    <section class="content-grid">
      <div class="panel stack">
        <div class="panel-head">
      ...
      <div class="panel stack">
        <div class="panel-head">
          <div>
            <h3>导出设置</h3>
            <div class="subtle">这些选项会跟着项目一起保存，导出 Markdown 时直接生效。</div>
          </div>
        </div>
        <div class="note">
          <h4>Markdown 导出项</h4>
          <div class="stack">
            ${checkboxField("includeSynopsis", "包含作品简介", exportOptions.includeSynopsis, "export")}
            ${checkboxField("includeVolumes", "包含卷信息", exportOptions.includeVolumes, "export")}
            ${checkboxField("includeChapterSummaries", "包含章节摘要", exportOptions.includeChapterSummaries, "export")}
            ${checkboxField("includeAppendix", "包含附录", exportOptions.includeAppendix, "export")}
          </div>
        </div>
        <div class="note">
          <h4>导出结构</h4>
          <div class="subtle">当前支持导出标题、作品简介、卷信息、正文、章节摘要，以及角色卡/地点/势力/规则附录。</div>
        </div>
      </div>
    </section>
  `;
...
function checkboxField(name, label, checked, scope = "setup") {
  return `
    <label class="checkbox-field">
      <input type="checkbox" data-scope="${scope}" data-field="${name}" ${checked ? "checked" : ""} />
      <span>${label}</span>
    </label>
  `;
}
...
function renderRewriteReview(review, chapter) {
  return `
    <div class="note rewrite-review">
      <div class="panel-head">
        <div>
          <h4>最近一次改写对比</h4>
          <div class="subtle">第 ${chapter.index} 章 · ${escapeHtml(review.label)} · ${escapeHtml(
            rewriteScopeText(review.scope)
          )} · ${escapeHtml(formatDate(review.updatedAt))}</div>
        </div>
        <div class="toolbar">
          <button class="secondary" data-action="undo-last-rewrite">撤销改写</button>
          <button class="secondary" data-action="dismiss-rewrite-review">关闭对比</button>
        </div>
      </div>
      <div class="rewrite-compare">
        <div class="memory-item">
          <h4>改写前</h4>
          <div class="subtle">${escapeHtml(review.beforePreview)}</div>
        </div>
        <div class="memory-item">
          <h4>改写后</h4>
          <div class="subtle">${escapeHtml(review.afterPreview)}</div>
        </div>
      </div>
      <div class="subtle">只有当本章在这次改写之后没有再被继续修改时，系统才会自动撤销。</div>
    </div>
  `;
}

function rewriteScopeText(scope) {
  if (scope === "manual") return "选中片段";
  if (scope === "ending") return "自动识别的结尾段落";
  if (scope === "chapter-fallback") return "整章 fallback";
  return "整章";
}
