...
import { SendIcon, PinIcon, XIcon, SparklesIcon, UserIcon, EditIcon, EyeIcon } from 'lucide-react';
...
interface MiddlePanelProps {
  ...
  onPinDocument: (content: string) => void | Promise<void>;
  onCloseDoc: () => void;
}
...
export function MiddlePanel({
  ...
  const handlePin = () => {
    void onPinDocument(editContent);
  };
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              style={{
                background: editMode ? '#F0F0F5' : 'transparent',
                border: '1px solid #E0E0EA',
                color: '#6E6E8A',
              }}
              onMouseEnter={(e) => (e.currentTarget as HTMLElement).style.background = '#F0F0F5'}
              onMouseLeave={(e) => (e.currentTarget as HTMLElement).style.background = editMode ? '#F0F0F5' : 'transparent'}
            >
              {editMode ? <EyeIcon size={12} /> : <EditIcon size={12} />}
              {editMode ? '预览' : '编辑'}
            </button>
            {activeDoc.type !== 'characterDoc' && (
              <button
      ...
      <div className="flex-shrink-0 p-4" style={{ borderTop: '1px solid #EAEAEA' }}>
        {activeDoc && (
          <p className="text-xs mb-2.5" style={{ color: '#9999B3' }}>
            {activeDoc.type === 'characterDoc'
              ? '您可以询问 AI 关于角色的任何问题'
              : `针对「${activeDoc.title}」的修改建议将自动创建修改类对话`}
          </p>
        )}
        <div
