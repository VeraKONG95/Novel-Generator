...
export function ProjectPage() {
  ...
  const resolveConversationDoc = (conversation: Conversation | null) => {
    if (!conversation || conversation.type !== 'modification') {
      return null;
    }

    if (!conversation.relatedDocId || !conversation.relatedDocType) {
      return null;
    }

    if (conversation.relatedDocType === 'outline') {
      return {
        docId: conversation.relatedDocId,
        docType: 'outline' as const,
        docTitle: conversation.relatedDocTitle || '故事大纲',
        content:
          conversation.messages
            .slice()
            .reverse()
            .find((message) => message.proposal?.docType === 'outline')
            ?.proposal?.content || outline
      };
    }

    if (conversation.relatedDocType === 'chapter') {
      const chapter = currentProject.chapters.find((item) => item.id === conversation.relatedDocId);
      if (!chapter) return null;
      const latestProposal = conversation.messages
        .slice()
        .reverse()
        .find((message) => message.proposal?.docType === 'chapter');
      return {
        docId: chapter.id,
        docType: 'chapter' as const,
        docTitle: conversation.relatedDocTitle || `第${chapter.index}章：${chapter.title}`,
        chapter,
        content: latestProposal?.proposal?.content || chapter.content
      };
    }
  ...
  const appendConversationMessage = (conversationId: string, message: Message) => {
    setConversations((current) =>
      current.map((conversation) =>
        conversation.id === conversationId
          ? {
              ...conversation,
              preview: message.role === 'user' ? message.content.slice(0, 40) : conversation.preview,
  ...
  const handleSendMessage = async (text: string) => {
    ...
    const targetFromActiveDoc =
      activeDoc && activeDoc.type !== 'characterDoc'
        ? {
            docId: activeDoc.id,
            docType: activeDoc.type as 'outline' | 'chapter',
            docTitle: activeDoc.title,
            chapter:
              activeDoc.type === 'chapter'
                ? currentProject.chapters.find((item) => item.id === activeDoc.id)
                : null,
            content: activeDoc.content
          }
        : null;
    const targetFromConversation = resolveConversationDoc(selectedConversation);
    const modificationTarget = targetFromActiveDoc || targetFromConversation;
    const isModContext = Boolean(modificationTarget);
    ...
    if (isModContext && modificationTarget && currentProject) {
      const conversationId =
        selectedConversation?.type === 'modification' ? selectedConversation.id : generateId();

      if (selectedConversation?.type === 'modification') {
        appendConversationMessage(conversationId, userMsg);
      } else {
        const newConversation: Conversation = {
          id: conversationId,
          type: 'modification',
          title: `${modificationTarget.docTitle} 修改`,
          preview: text.slice(0, 40),
          timestamp: new Date().toISOString(),
          messages: [userMsg],
          relatedDocId: modificationTarget.docId,
          relatedDocType: modificationTarget.docType,
          relatedDocTitle: modificationTarget.docTitle
        };
      ...
      try {
        const rewriteChapter =
          modificationTarget.docType === 'chapter' && modificationTarget.chapter
            ? {
                ...modificationTarget.chapter,
                content: modificationTarget.content,
                instruction: text
              }
            : {
                id: modificationTarget.docId,
                index: 0,
                title: modificationTarget.docTitle,
                goal: '',
                summary: currentProject.blueprint.synopsis || currentProject.setup.premise || '',
                content: modificationTarget.content,
                instruction: text,
                status: 'draft',
        ...
        const result = await window.novalAPI.rewriteText({
          ...
          chapter: rewriteChapter,
          mode: modificationTarget.docType === 'chapter' ? 'rewrite_chapter' : 'rewrite',
          target: 'chapter',
          targetText: modificationTarget.content,
          targetOrigin: 'manual'
        });
        ...
        const aiMsg: Message = {
          ...
          role: 'ai',
          content: result?.ok
            ? String(result.data?.content || '').trim() || '本次修改没有返回可用结果。'
            : '这次修改没有成功返回结果，请稍后重试。',
          timestamp: new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }),
          proposal: result?.ok
            ? {
                docId: modificationTarget.docId,
                docType: modificationTarget.docType,
                docTitle: modificationTarget.docTitle,
                content: String(result.data?.content || '').trim(),
                operation: String(result.data?.operation || '修改稿'),
                status: 'pending'
              }
            : undefined
        };

        appendConversationMessage(conversationId, aiMsg);
      } catch (error) {
        appendConversationMessage(conversationId, {
          id: generateId(),
          role: 'ai',
          content: `修改失败：${error instanceof Error ? error.message : '未知错误'}`,
          timestamp: new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })
        });
      } finally {
        setIsGenerating(false);
    ...
    window.setTimeout(() => {
      ...
      appendConversationMessage(newConvId, aiMsg);
      setIsGenerating(false);
    }, 900);
  };
  ...
  const handlePinProposal = async (messageId: string) => {
    ...
    if (proposal.docType === 'outline') {
      nextProject = updateOutline(currentProject, proposal.content);
      updateCurrentProject(nextProject);
    } else {
      nextProject = updateChapterContent(currentProject, proposal.docId, proposal.content);
      nextProject = await analyzeChapterInProject(nextProject, proposal.docId);
      updateCurrentProject(nextProject);
    }
    ...
    showNotification(
      proposal.docType === 'outline'
        ? '✓ 这一版大纲已 Pin 到右侧成品区'
        : '✓ 这一版章节已 Pin 到右侧成品区，并同步更新摘要与记忆'
    );
  };
