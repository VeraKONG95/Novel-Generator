...
export function HomePage() {
  ...
  const currentDraftProjects = projects.filter((project) => project.source === 'current');
  const draftBoxProjects = projects.filter((project) => project.source === 'draft');
  const recentProjectsList = projects.filter((project) => project.source === 'recent');
  ...
  const renderProjectCard = (project: Project) => {
    ...
    return (
      <div
        key={`${project.source || 'project'}-${project.filePath || project.draftId || project.id}`}
  ...
  const renderProjectSection = (title: string, description: string, items: Project[]) => {
    ...
    return (
      <section className="mb-8 last:mb-0">
        <div className="mb-3">
          <h2 style={{ color: '#1A1A2E' }} className="text-lg">
            {title}
          </h2>
        ...
        {!isReady ? (
          ...
          <>
            {renderProjectSection(
              '当前草稿',
              '你当前正在编辑但还未保存为文件的项目。',
              currentDraftProjects
            )}
            {renderProjectSection(
              '草稿箱',
              '切换项目或新建项目时自动保留下来的未保存草稿。',
              draftBoxProjects
            )}
            {renderProjectSection(
              '最近项目',
              '已经导入或保存过的项目，会按最近访问时间展示。',
              recentProjectsList
            )}
          </>
        )}
